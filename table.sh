...
grep '<h2><abbr title="Premier League"><span>Premier League</span></abbr></h2>' $WDOWNLOAD_PATH/table.shtml | sed -e 's/\(^.*<span>Premier League<\/span><\/abbr><\/h2> \)\(.*\)\(<span>Championship<\/span>.*$\)/\2/' | sed -e 's/\(^.*<\/thead> <tbody>\)\(.*\)\(<\/tbody> <\/table>\)\(.*$\)/\2/' | sed -e 's/<\/tr>/\n/g' | sed -e 's/<[^>]*>/\t/g' | tr -s '\t' '\t' | cut -sf 3,6,8,10,12,14,16,18 > $WDOWNLOAD_PATH/table.$WDATETIME
